...
class LoadTestConfig(lg.Config):
    duration: float = 30.0
    dynamic_size: int = 8  # bytes
    output_file: str = dataclasses.field(default_factory=lambda: str(
        pathlib.Path(tempfile.gettempdir()) / pathlib.Path(
            f"load_test_python_{PLATFORM}_{get_datestamp()}.h5")))
    sample_rate: float = SAMPLE_RATE  # rate at which to generate samples
...
class LoadTestPublisher(lg.Node):
    ...
    async def publish(self) -> lg.AsyncPublisher:
        ...
        while self.running:
            ...
            if current_time - last_time >= 1 / self.config.sample_rate:
                ...
                yield self.OUTPUT, LoadTestSample(
                    ...
                    dynamic_data=np.ones(
                        shape=(self.config.dynamic_size //
...
class LoadTestSubscriberState(lg.State):
    ...
    received_data: List[Tuple[float, float, int,
...
class LoadTestSubscriber(lg.Node):
    ...
    async def log_result(self) -> None:
        ...
        min_timestamp = min(timestamp
                            for timestamp, _, _, _ in self.state.received_data)
        max_timestamp = max(timestamp
                            for timestamp, _, _, _ in self.state.received_data)
        total_latency = sum(receive_timestamp - send_timestamp
                            for send_timestamp, receive_timestamp, _, _ in
                            self.state.received_data)

        latencies = [
            receive_timestamp - send_timestamp for send_timestamp,
            receive_timestamp, _, _ in self.state.received_data
        ]
        ...
        with h5py.File(self.config.output_file, mode="a") as output_file:
            ...
            if sample_summary_dataset_name not in output_file:
                ...
                sample_summary_dataset = output_file[
                    sample_summary_dataset_name]
            ...
            sample_summary_dataset[-num_samples:] = [(
                ...
                received_timestamp,
                counter,
            ) for sent_timestamp, received_timestamp, counter, _ in
                                                     self.state.received_data]
...
def run_many_dynamic_sizes() -> None:
    ...
    for dynamic_size in dynamic_sizes:
        ...
        runner = lg.ParallelRunner(graph=LoadTest(config=config.replace(
