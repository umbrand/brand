...
def numpy_export(threshDict, filtDict, threshArray, filtArray,rConnection):
    ...
    for ii in threshDict.keys():
        if 'chan' in ii.decode():
            ...
            filtDict[ii] = pack('h',int(filtArray[chInd,:]))
            chInd += 1
...
thresholds = calc_thresh(r, threshMult, readCalls, numSamples, numChannels) # get the array
...
while True:
   ...
   numpy_export(threshDict, xread_dict, threshCross, filtBuffer, r)
