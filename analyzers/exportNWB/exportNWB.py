...
def add_stream_sync_timeseries(nwbfile, stream, time_data):
    ...
    time_data_stack = {k:time_data[k] for k in time_data if k != 'sync_timestamps'}
    column_order_string = ', '.join([k for k in time_data.keys() if k != 'sync_timestamps'])
...
def create_nwb_trials(nwbfile, stream, stream_data, var_params):
    ...
    for var in var_params:
        if 'nwb' in var_params[var] and var_params[var]['nwb'] is not None and 'trial_state' in var_params[var]['nwb']:
            trial_state = var_params[var]['nwb']['trial_state']

    # first get trial indicators
    start_trial_indicators = var_params[trial_state]['nwb']['start_trial_indicators']
    end_trial_indicators = var_params[trial_state]['nwb']['end_trial_indicators']
    other_trial_indicators = var_params[trial_state]['nwb']['other_trial_indicators']
    ...
    for s_time, e_time, s_ind, e_ind in zip(start_times, end_times, start_inds, end_inds):
        # first find the other_times corresponding to the current trial
        trial_other_times = {k:other_times[k][np.logical_and(other_times[k] >= s_time, other_times[k] <= e_time)]
                                for k in other_times}
        trial_other_times = {k:np.nan   if trial_other_times[k].size == 0
                                        else trial_other_times[k][0]
...
try:
    ...
    logging.error(f"No model published to supergraph_stream in Redis. Exiting.")
    sys.exit(1)
...
redis_inputs = model_data['analyzers']['exportNWB']['redis_inputs']
redis_outputs = model_data['analyzers']['exportNWB']['redis_outputs']
...
stream_dict = {k:{'source':None, 'source_yaml':None, 'config':{}, 'last_id':b'-'}
    ...
    if stream_dict[stream]['source'] is None:
        ...
        if 'enable_nwb' in stream_dict[stream]['config'] and 'type_nwb' in stream_dict[stream]['config']:
            if stream in exportnwb_enables:
                stream_dict[stream]['config']['enable_nwb'] = exportnwb_enables[stream]
            else:
                logging.info(f'Using default NWB enable. Stream: {stream}')
        else:
            logging.warning(f'Invalid NWB parameters. \'enable_nwb\' and \'type_nwb\' are required. Stream: {stream}')
...
for stream in stream_dict:
    ...
    if stream_dict[stream]['config']['enable_nwb'] and stream_dict[stream]['config']['type_nwb'] == 'Trial':
...
for implant in participant_implants:
    ...
    if device['name'] in nwbfile.devices:
        ...
        nwb_device = nwbfile.create_device(
                                name            = device['name'],
...
for stream in stream_dict:
    ...
    if stream_dict[stream]['config']['enable_nwb']:
        ...
        stream_data = {k:{  'data'              : np.empty( (stream_len*stream_dict[stream]['config'][k]['samp_per_stream'], stream_dict[stream]['config'][k]['chan_per_stream']),
                                                            dtype=object if stream_dict[stream]['config'][k]['sample_type'] == 'str' else stream_dict[stream]['config'][k]['sample_type']), # ugly, but need to handle strings
                            'sync_timestamps'   : np.empty( stream_len*stream_dict[stream]['config'][k]['samp_per_stream'],
                                                            dtype=np.double),
                            'sample_count'      : 0}
        ...
        time_data = {   'sync_timestamps'   : np.empty(stream_len, dtype=np.double),    # sync timestamps
                        'monotonic_ts'      : np.empty(stream_len, dtype=np.double),    # monotonic timestamp
        ...
        while entry_count < stream_len:
            ...
            for ind, entry in enumerate(stream_read):
                ...
                time_data['sync_timestamps'][entry_count+ind] = float(sync_data[sync_name])/1000 # get blocked sync timestamp in ms, convert to seconds
                time_data['monotonic_ts'][entry_count+ind] = float(entry[1][time_key.encode()])
                time_data['redis_ts'][entry_count+ind] = float(entry[0].decode('utf-8').split('-')[0])/1000

                # get other sync signals for this entry
                for sync in sync_data:
                    if sync in sync_name:
                        continue
                    time_data[sync][entry_count+ind] = float(entry[sync])/1000

                for var in stream_data:
                    if 'nwb' not in stream_dict[stream]['config'][var].keys():
                        continue
                    var_config = stream_dict[stream]['config'][var]
                    batch_idx = list(range(stream_data[var]['sample_count'], stream_data[var]['sample_count']+var_config['samp_per_stream']))
                    if stream_dict[stream]['config'][var]['sample_type'] == 'str':
                        stream_data[var]['data'][batch_idx, :] = entry[1][var.encode()].decode('utf-8')
                    else:
        ...
        nwb_funcs[stream_dict[stream]['config']['type_nwb']](
            ...
            stream,
            stream_data,
            {k:stream_dict[stream]['config'][k] for k in stream_dict[stream]['config'] if k not in ['enable_nwb', 'type_nwb', 'sync']})
