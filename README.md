...
The supergraph will pull Git commit hashes for each node to easily track the exact code version used. `booter` machines (see [Multi-machine graphs](#multi-machine-graphs) below) will check their local Git hashes against those in the supergraph to validate the expected code versions are being launched. If a node is not located in a Git repository, then its Git hash in the supergraph will be empty.
